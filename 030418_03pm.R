# Things to do:
  # Zero-fill the data after species is selected
  # Add digital elevation model 
    # https://mapzen.com/documentation/terrain-tiles/formats/
    #https://cran.r-project.org/web/packages/elevatr/vignettes/introduction_to_elevatr.html#mapzen_terrain_tile_service
    # Consider geoGAM
      # https://cran.r-project.org/web/packages/geoGAM/geoGAM.pdf
  # Figure out how to make interval ggplot
...
if(T){
  sp.x = hi[hi$species=="Japanese White-eye",]
}
...
if(T){
  myColors <- (heat.colors(max(sp.x$nseen), alpha = 1))
...
if(T){
  ...
  countInt = as.data.frame(attr(q5Colours,"table")) 
  colCounts = as.character(countInt[countInt$Freq!=0,1])
  
  sp.x.t = sp.x[,1:4]
  sp.x.t$col = as.vector(q5Colours)
  
  ggplot(sp.x.t, aes(sp.x.t$lng, sp.x.t$lat, col=factor(sp.x.t$col))) + # Something in here is flipping the colors
    geom_point() + scale_color_manual(name = "Count",
                                      labels = colCounts, 
                                      values=attr((q5Colours), "palette")) +
    ggtitle(paste(unique(sp.x$species), "Recorded Abundance",sep = " ")) +
    theme(panel.background = element_rect(fill = 'lightgray'),
          panel.grid.major = element_line(colour = 'white'),
          panel.grid.minor = element_line(colour = 'white'),
          plot.title = element_text(face = 'bold', hjust = 0.5, family = 'sans'))
...
if(T){
  ...
  q5 = classIntervals(sp.x$nseen, n=5, style = "quantile")
  q5Colours = findColours(q5,pal)
  plot(sp.x$lng, sp.x$lat, col = q5Colours, 
       pch = 19, axes = T, cex = 0.7, main = "nseen",
       xlim = c(-156.52,-154.6))
